...
const EMAIL_STYLES = `
  ...
  .cell-label { display: table-cell; padding: 6px 12px 6px 0; color: #94a3b8; font-size: 14px; width: 35%; }
...
export function buildOwnerAlertEmail({ bookingId, submissionTime, fullName, email, phone, carType, place, message }) {
  return `
    ...
    <body>
      <div class="container">
        <div class="header">
          <h1>🚗 New Travel Enquiry Received!</h1>
        ...
        <div class="content">
          ...
          <div class="grid">
            <div class="row"><div class="cell-label">Name:</div><div class="cell-val"><strong>${fullName}</strong></div></div>
          ...
          <div class="section-title">🚘 Trip Preference</div>
          <div class="grid">
            <div class="row"><div class="cell-label">Preferred Car:</div><div class="cell-val"><strong>${carType || 'Not Sure / Recommend'}</strong></div></div>
            <div class="row"><div class="cell-label">Destination / Place:</div><div class="cell-val">${place || 'Not Sure / Other'}</div></div>
          </div>

          ${message ? `
          <div class="highlight-box">
            <strong style="color: #f59e0b;">Message from Customer:</strong><br/>
...
export function buildCustomerConfirmationEmail({ bookingId, fullName, carType, place }) {
  ...
  return `
    ...
    <body>
      <div class="container">
        <div class="header" style="background: linear-gradient(135deg, #f59e0b, #d97706); color: #090d16;">
          <h1 style="color: #090d16;">Thanks for Reaching Out!</h1>
          <p style="color: #090d16; font-weight: bold; opacity: 1;">Jit Tours and Travels &bull; UP & MP Tour & Taxi Services</p>
        </div>
        <div class="content">
          <p>Namaste <strong>${fullName}</strong>,</p>
          <p>Thank you for your enquiry with <strong>Jit Tours and Travels</strong> (Ref: <strong style="color: #f59e0b;">${bookingId}</strong>). Our team has been notified and will get back to you shortly with availability and a final quotation.</p>

          <div class="grid" style="background: #0f172a; border: 1px solid #334155; border-radius: 8px; padding: 12px 16px;">
            <div class="row"><div class="cell-label">Reference ID:</div><div class="cell-val" style="color: #f59e0b;">${bookingId}</div></div>
            <div class="row"><div class="cell-label">Preferred Car:</div><div class="cell-val">${carType || 'To be recommended'}</div></div>
            <div class="row"><div class="cell-label">Destination:</div><div class="cell-val">${place || 'To be discussed'}</div></div>
          </div>

          <a href="${waUrl}" class="btn-confirm" target="_blank">
            💬 CHAT WITH US ON WHATSAPP
          </a>

          <p style="font-size: 12px; color: #94a3b8; text-align: center; margin-top: 12px;">
            Or call our helpline directly: <strong style="color: #f8fafc;">${OWNER_PHONE_DISPLAY}</strong>
          </p>
        </div>
        <div class="footer">
